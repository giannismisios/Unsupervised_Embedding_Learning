...
def eval_nmi(embedding, label, normed_flag=False, fast_kmeans=False):
    ...
    if fast_kmeans:
        kmeans = KMeans(n_clusters=num_category, n_init=1)  # g n_jobs=8
    else:
        kmeans = KMeans(n_clusters=num_category)  # g n_jobs=8
...
def plot_closer_K(embedding, label, testloader, K_list =[4]):
    img_array = testloader.dataset.img_data
    ...
    for i in range(0,embedding.shape[0],30):
        ...
        index = np.argsort(dis)
        plot_labels = label[i]
        plot_img = img_array[i]
        for k in range(np.max(K_list)):
            plot_labels = np.append(plot_labels, label_array[index[k]])
            plot_img = np.vstack((plot_img, img_array[index[k]]))
        plot_img = np.reshape(plot_img, (k + 2, img_array[0].shape[0], img_array[0].shape[1], img_array[0].shape[2]))

        fig, axes = plt.subplots(1, np.max(K_list)+1, figsize=(25 / 6 * np.max(K_list)+1, 25), squeeze=False, constrained_layout=True)
        for j in range(plot_labels.shape[0]):
            ax = axes[0, j]
            ax.imshow(plot_img[j])
