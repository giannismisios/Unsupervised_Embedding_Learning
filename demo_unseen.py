...
# Training
def train(epoch, optimizer, net, trainloader, criterion, freeze_bn, writer):
    print('\nEpoch: %d' % epoch)
    ...
    for batch_idx, (inputs1, inputs2, targets, indexes) in enumerate(trainloader):
        ...
        inputs1, inputs2, targets, indexes = inputs1.to(device), inputs2.to(device), targets.to(device), indexes.to(
            device)
