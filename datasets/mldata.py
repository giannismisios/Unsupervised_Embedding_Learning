...
class MLDataInstance(data.Dataset):
    ...
    def __init__(self, src_dir, dataset_name, train=True, transform=None, target_transform=None, nnIndex=None):
        ...
        if train:
            img_data = np.load(data_dir + '{}_{}_256resized_img.npy'.format('training',
                                                                            dataset_name))  # g add ,mmap_mode='r' for ebay dataset
            img_label = np.load(data_dir + '{}_{}_256resized_label.npy'.format('training',
                                                                               dataset_name))  # g add ,mmap_mode='r' for ebay dataset
        else:
            img_data = np.load(data_dir + '{}_{}_256resized_img.npy'.format('validation',
                                                                            dataset_name))  # g add ,mmap_mode='r' for ebay dataset
            img_label = np.load(data_dir + '{}_{}_256resized_label.npy'.format('validation',
                                                                               dataset_name))  # g add ,mmap_mode='r' for ebay dataset
